...
#include "tools/gn/jumbo_writer.h"
...
// TYPE_NONE if |source| doesn't need any build command.
Toolchain::ToolType PrepareCompilerBuildLine(
    const Target& target,
  ...
  if (!target.GetOutputFilesForSource(source, &tool_type, tool_outputs)) {
    ...
    return tool_type;  // No output for this source.
  }
...
bool IsJumboEnabledForTarget(const Target* target) {
...
// reference to it.
const Target::FileList& GetSourcesForTarget(const Target* target,
