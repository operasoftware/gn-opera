...
const char* PrepareCompilerBuildLine(
  ...
  if (!target.GetOutputFilesForSource(source, &tool_name, tool_outputs)) {
    ...
    return nullptr;  // No output for this source.
  }
  ...
  for (const auto& module_dep : module_dep_info) {
    if ((*tool_outputs)[0] != module_dep.pcm)
      extra_deps->push_back(module_dep.pcm);
  }
...
void NinjaCBinaryTargetWriter::WriteSources(
  ...
  for (const auto& source : sources) {
    ...
    deps.resize(0);
    const char* tool_name =
        PrepareCompilerBuildLine(*target_, source, pch_deps, input_deps,
                                 module_dep_info, &deps, &tool_outputs,
                                 other_files);
    if (!tool_name)
    ...
    if (!source.IsModuleMapType() && !tool_outputs.empty())
      ...
      for (const auto* source : jumbo_file.second) {
        ...
        const char* tool_name =
            PrepareCompilerBuildLine(*target_, *source, pch_deps, input_deps,
                                     module_dep_info, &deps, &tool_outputs,
                                     other_files);
        if (tool_name && tool_name != Tool::kToolNone) {
          WriteCompilerBuildLine({*source}, deps, order_only_deps, tool_name,
                                 tool_outputs);
        }
      }
